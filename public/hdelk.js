...
var hdelk = (function(){
	...
	// edit by Pranav: edited some of the variables
    ...
    var node_fill_color = '#ddddff';
    var node_stroke_color = '#000';
    var node_highlight_stroke_width = 2;
    var node_stroke_width = 1;
    var node_name_text_color = '#000';
    ...
    var edge_width = 1;
    var edge_color = '#888';
    var edge_highlight_color = ["#ff0000", "#ffa500", "#008000", "#0000ff", "#ee82ee"];
    var edge_highlight_width = 3;
...
/*
    ...
    var node = function( draw, child, offsetX, offsetY ) {
        ...
        node_body( group, child.id, child.x + offsetX, child.y + offsetY, child.width, child.height, childColor, child.highlight, node_stroke_color, child.constant );
        ...
        if ( ports ) {
            ports.forEach( function( item, index ){                
				...
				// edit by pranav: separate mc pins/constants from sc pins
				
				let arrow_ind = portText.indexOf('\u2192');
				let direction = item["layoutOptions"]["elk.port.side"];
				let real_portTextItem;
				...
				if (arrow_ind == -1) {
					...
					real_portTextItem = group.text(portText).style("font-size:"+port_name_font_size).fill({color:nameColor});
				}
				else {
					// non-port text
					let input_extra_text = portText.slice(0, arrow_ind + 2);
					let output_extra_text = portText.slice(arrow_ind - 1);
					
					// port text
					let input_port_text = portText.slice(arrow_ind + 2);
					let output_port_text = portText.slice(0, arrow_ind - 1);					
					
					let dummy_portTextItem, shift;
					
					if (direction == 'WEST') {
						dummy_portTextItem = group.text(input_extra_text).style("font-size:"+port_name_font_size).fill({color:nameColor});
						shift = dummy_portTextItem.node.getComputedTextLength();

						group.rect(item.width - shift, item.height).move(shift + offsetX + child.x+item.x,offsetY + child.y+item.y)
														   .attr({ fill:fillColor, 'stroke-width': strokeWidth, stroke:strokeColor })
														   .stroke({width:strokeWidth});

						real_portTextItem = group.text(function(add) {
							add.tspan(input_extra_text).style("font-size:"+port_name_font_size).fill({color:"#000"});
							add.tspan(input_port_text).style("font-size:"+port_name_font_size).fill({color:nameColor});
						});
					}
						...
						group.rect(item.width, item.height - shift).move(offsetX + child.x+item.x, shift + offsetY + child.y+item.y)
														   .attr({ fill:fillColor, 'stroke-width': strokeWidth, stroke:strokeColor })
														   .stroke({width:strokeWidth});	
						...
						dummy_portTextItem = group.text(output_extra_text).style("font-size:"+port_name_font_size).fill({color:nameColor});
						shift = dummy_portTextItem.node.getComputedTextLength();

						group.rect(item.width - shift, item.height).move(offsetX + child.x+item.x,offsetY + child.y+item.y)
														   .attr({ fill:fillColor, 'stroke-width': strokeWidth, stroke:strokeColor })
														   .stroke({width:strokeWidth});

						real_portTextItem = group.text(function(add) {
							add.tspan(output_port_text).style("font-size:"+port_name_font_size).fill({color:nameColor});							
							add.tspan(output_extra_text).style("font-size:"+port_name_font_size).fill({color:"#000"});
						});														   
					}
					else {
						dummy_portTextItem = group.text(output_extra_text).style("font-size:"+port_name_font_size).fill({color:nameColor});
						...
						group.rect(item.width, item.height - shift).move(offsetX + child.x+item.x, offsetY + child.y+item.y)
														   .attr({ fill:fillColor, 'stroke-width': strokeWidth, stroke:strokeColor })
														   .stroke({width:strokeWidth});	
					...
					dummy_portTextItem.clear();
				}
				
				
				var portTextWidth = real_portTextItem.node.getComputedTextLength();
                if ( item.vertical ) {
                    //group.rect(item.width, item.height).move(offsetX + child.x+item.x,offsetY + child.y+item.y)
                    ...
                    real_portTextItem.transform( { rotation:90, cx:0, cy:0  } ).move( offsetY + child.y+item.y+(item.height-portTextWidth)/2, -(offsetX + child.x+item.x+item.width-(item.width-port_name_font_size)/2 + 2) );
                }
                else {
                    ...
                    //var portTextWidth = portTextItem.node.getComputedTextLength();
                        // draw the background
                    real_portTextItem.move(offsetX + child.x+item.x+(item.width-portTextWidth)/2, offsetY + child.y+item.y + 2);
                }
            } )
        }
    ...
    var edge = function( draw, edge, offsetX, offsetY ) {
		...
		// edit by Pranav: made edges black
		//color = "#000";
